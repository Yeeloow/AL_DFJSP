...
def run_single_step_debug():
    """
    단 한 스텝만 진행하며 모든 중간 과정을 상세히 출력하여 검증합니다.
    """
    print("--- 스텝별 디버깅 시작 ---")

    # --- 1. 테스트용 문제 및 Pre-occupied Schedule 정의 ---
    
    # 에이전트가 새로 스케줄링해야 할 작업
    new_jobs_instance = [
        [([0, 1], [10, 15])], # Job 0: Op 0
        [([0, 1], [8, 12])]  # Job 1: Op 0
    ]
    # 기계에 이미 예약된 배경 작업들
    pre_occupied_schedule = {
        0: [(-1, -1, 10, 20)], # Machine 0: 10초부터 20초까지 다른 일로 바쁨
        1: []                 # Machine 1: 비어있음
    }
    ...
    # 에이전트는 무작위 결정을 하도록 가중치를 불러오지 않음 (초기 상태)
    ...
    # reset 함수에 pre-occupied schedule 전달
    ...
    # Agent 2의 결정 (내부 계산 과정 확인)
    print("\nAgent 2의 내부 계산 과정:")
    # 에이전트 2 내부의 earliest_fit_time 계산 로직을 직접 호출하여 확인
