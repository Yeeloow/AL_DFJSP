...
class Machine:
    def __init__(self, machine_id):
        self.id = machine_id; self.available_time = 0.0; self.op_history = []
    @property
    def utilization_rate(self):
        if self.available_time == 0: return 0.0
        work_time = sum(op.machine_map[self.id] for op, _, _ in self.op_history if self.id in op.machine_map)
...
class FJSPEnv:
        ...
        if initial_schedule:
            ...
            for machine_id, schedule_list in initial_schedule.items():
                ...
                for job_id, op_id, start_time, end_time in schedule_list:
                    # pre-occupied 스케줄은 실제 Operation 객체가 아니므로, 
                    # 시간 정보만 반영하기 위해 가상의 op 객체를 생성합니다.
                    dummy_op_data = ([machine_id], [end_time - start_time])
                    dummy_op = Operation(op_id, job_id, dummy_op_data)
                    machine.op_history.append((dummy_op, start_time, end_time))
                    machine.available_time = max(machine.available_time, end_time)
                    max_time = max(max_time, end_time)
                machine.op_history.sort(key=lambda x: x[1])
    ...
    def _get_state(self):
        ...
        for m_idx in range(self.n_machines):
            ...
            machine_features_list.append([
                self.n_machines, 1 if machine.available_time > self.current_time else 0, 
                machine.available_time - self.current_time, machine.available_time, num_candidates, 
                machine.utilization_rate, avg_utilization
            ])
        ...
        for machine in self.machines:
            for i in range(len(machine.op_history) - 1):
                u_op, v_op = machine.op_history[i][0], machine.op_history[i+1][0]
                edge_index_list.append([self.op_map.get((u_op.job_id, u_op.id)), self.op_map.get((v_op.job_id, v_op.id))])
        
        edge_index_list = [edge for edge in edge_index_list if edge[0] is not None and edge[1] is not None]
        ...
        return {
            ...
            'op_histories': [m.op_history for m in self.machines]
        }
    ...
    def step(self, action):
        ...
        prev_op_comp_time = 0 if op.id == 0 else self.jobs[job_id].ops[op.id - 1].completion_time
        ...
        machine.op_history.append((op, start_time, completion_time)); machine.op_history.sort(key=lambda x: x[1])
        
        if any(op.is_scheduled for job in self.jobs for op in job.ops):
            if any(m.op_history for m in self.machines):
                self.current_time = min(m.available_time for m in self.machines if m.op_history)
